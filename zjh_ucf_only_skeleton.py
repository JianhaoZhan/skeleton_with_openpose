...
try:
    ...
    image_dir='/New-8T/mars/UCF101'
    gpu=1
    with open('list_ucf.txt','r+') as f:
        line=f.readline()
        while line:
            target=os.path.join('/New-8T/mars/UCF101_skeleton',line.split('.')[0])
            org=os.path.join(image_dir,line.split('.')[0])
            org_num=os.listdir(os.path.join(image_dir,line.split('.')[0]))
            ...
            for i in range(1,len(org_num)+1):
                if '{:0>5d}.jpg'.format(i) not in now_num:
                    try:
                        ...
                        imageToProcess = cv2.imread('./zero_ucf.jpg')
                        ...
                        cv2.imwrite(r'{}/{:0>5d}.jpg'.format(target, i), imageToProcess)
