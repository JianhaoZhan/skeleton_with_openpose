    ...
    for i in os.listdir(label):
        if "split1.txt" in i:
            ...
            with open(i,'r+') as f:
                line=f.readline()
                while line:
                    target=os.path.join('/New-8T/mars/HMDB51_skeleton', i.split('/')[-1].split('_test_split1.txt')[0],line.split('.')[0])
                    ...
                    for j in range(1,len(org_num)//3+2):
                        if '{:0>5d}.jpg'.format(j) not in now_num:
                            try:
                                ...
                                #cv2.waitKey(0)
                                imageToProcess = cv2.imread('./zero_hmdb.jpg')
                                ...
                                #cv2.imshow("OpenPose 1.7.0 - Tutorial Python API", imageToProcess)
                                #cv2.waitKey(0)
                                #print(r'{}/{:0>5d}.jpg'.format(target, j)),exit()
